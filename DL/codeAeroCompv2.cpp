...
bool UAeroNNComponentV2::EnsureInputShape(int32 BatchSize)
{
    ...
    InputShapes.Add(UE::NNE::FTensorShape::Make({ BatchSize, 5 }));
...
) const
{
    ...
    for (int32 i = 0; i < SurfaceSamples.Num(); ++i)
    {
        ...
        Fx += -Cp * S.Normal.X * S.DS;
        Fy += -Cp * S.Normal.Y * S.DS;
    }
...
bool UAeroNNComponentV2::EvaluateAeroCoefficients(float AlphaRad, FAeroCoefficientsV2& Coeffs)
{
    ...
    if (!EvaluateCpBatch(SurfaceSamples, AlphaRad, CpValues))
    {
        return false;
    }

    Coeffs = IntegratePressureCoefficients(CpValues, AlphaRad);
    return true;
}
