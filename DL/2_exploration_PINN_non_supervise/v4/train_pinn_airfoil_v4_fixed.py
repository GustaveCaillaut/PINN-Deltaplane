...
# ============================================================
# PINN Airfoil V4 fixed-case test
# ============================================================
# Purpose: test whether a more physically reasonable setup can produce lift
# without a CL loss.
#
# Tested changes:
#   1. Larger domain by default: x=[-4,6], y=[-3,3].
#   2. Inclined farfield BCs:
#        - inflow boundaries: impose freestream velocity
#        - outflow boundaries: impose Cp=0
#        - optional weak Cp=0 on all farfield boundaries
#   3. Automatic outward-normal check.
#   4. Default w_cl=0, so no direct CL anchor.
#   5. Higher Reynolds by default: U=2, nu=0.01, chord=1 -> Re=200.
#
# Model:
...
#
# Convention:
#   NACA airfoil centered around x=0
#   leading edge x=-0.5, trailing edge x=+0.5
#   freestream left -> right, inclined by alpha:
#       u_inf = U cos(alpha)
#       v_inf = U sin(alpha)
# ============================================================
...
parser.add_argument("--outdir", type=str, default="runs_pinn_airfoil_v4_fixed")
...
# NACA parameters, so we can optionally test stronger camber later.
...
# training
parser.add_argument("--adam-steps", type=int, default=8000)
...
parser.add_argument("--w-outflow-cp", type=float, default=5.0)
parser.add_argument("--w-far-cp", type=float, default=1.0)
...
def build_surface_geometry(x, y):
    ...
    # For clockwise polygons (area < 0), exterior is the left normal (-dy, dx).
    # For counterclockwise polygons, exterior is the right normal (dy, -dx).
    if area < 0:
        nx = -dy / ds
        ny = dx / ds
    else:
...
def check_normals(eps=1e-3):
    ...
    # Desired: plus outside, minus inside.
...
def compute_loss(curriculum_factor=1.0, small_batches=False):
    ...
    # Inflow velocity BC on actual inflow boundaries.
    ...
    # Outflow pressure reference on actual outflow boundaries.
    ...
    # Weak pressure reference on all farfield boundaries.
    # This keeps Cp from drifting, but with lower weight than outflow Cp.
...
def export_onnx(name="pinn_airfoil_v4_fixed.onnx"):
    model.eval()
...
def run_diagnostics(history):
    ...
    if history:
        keys = [
            ...
            "outflow_cp", "far_cp", "wall", "cl_loss"
        ]
        ...
        plt.ylabel("loss")
        plt.title("PINN V4 fixed losses")
        plt.tight_layout()
    ...
    with open(os.path.join(diag_dir, "summary.txt"), "w", encoding="utf-8") as f:
        f.write("PINN V4 fixed-case summary\n")
...
for step in range(1, args.adam_steps + 1):
    ...
    if step == 1 or step % args.print_every == 0:
        ...
        print(
            ...
            f"in_vel={row['inflow_vel']:.3e} | outCp={row['outflow_cp']:.3e} | "
            f"farCp={row['far_cp']:.3e} | wall={row['wall']:.3e} | "
            f"cl={row['cl']:+.3e} | cl_loss={row['cl_loss']:.3e} | cf={row['cf']:.2f} | "
            f"{elapsed:.1f}s"
        )

    if args.save_every > 0 and step % args.save_every == 0:
        save_checkpoint(f"pinn_airfoil_v4_fixed_step_{step}.pt", history)

save_checkpoint("pinn_airfoil_v4_fixed_adam.pt", history)
...
save_checkpoint("pinn_airfoil_v4_fixed.pt", history)
export_onnx()
