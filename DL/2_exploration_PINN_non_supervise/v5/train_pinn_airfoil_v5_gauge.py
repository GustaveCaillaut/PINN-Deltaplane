...
# ============================================================
# PINN Airfoil V5 fixed-case: pressure gauge test
# ============================================================
# Goal:
#   Test whether imposing Cp=0 on farfield boundaries was killing lift.
#   This version fixes pressure gauge at ONE point only:
#       Cp(x_ref, y_ref) = 0
#   and by default does NOT impose Cp=0 on entire boundaries.
#
# Main setup:
#   - fixed case f(x,y) -> (u,v,Cp)
#   - p = q Cp, q = 0.5 U^2
#   - larger domain default: [-4,6] x [-3,3]
#   - inclined inflow BC:
#       boundaries with U_inf · n < 0 -> impose freestream velocity
#   - optional outflow/far Cp losses, disabled by default
#   - no CL loss by default
#   - Re default: U=2, nu=0.01 -> Re=200
#
# Convention:
#   - chord c = 1
...
parser.add_argument("--outdir", type=str, default="runs_pinn_airfoil_v5_gauge")
...
def build_surface_geometry(x, y):
    ...
    # For clockwise polygons (area < 0), exterior is left normal (-dy, dx).
    # For counterclockwise polygons, exterior is right normal (dy, -dx).
    if area < 0:
        nx = -dy / ds
        ny = dx / ds
    else:
...
def compute_loss(curriculum_factor=1.0, small_batches=False):
    ...
    # Pressure gauge at a single point
...
def export_onnx(name="pinn_airfoil_v5_gauge.onnx"):
    model.eval()
...
def run_diagnostics(history):
    ...
    if history:
        ...
        plt.ylabel("loss")
        plt.title("PINN V5 pressure gauge losses")
        plt.tight_layout()
    ...
    with open(os.path.join(diag_dir, "summary.txt"), "w", encoding="utf-8") as f:
        f.write("PINN V5 pressure gauge fixed-case summary\n")
...
for step in range(1, args.adam_steps + 1):
    ...
    if args.save_every > 0 and step % args.save_every == 0:
        save_checkpoint(f"pinn_airfoil_v5_gauge_step_{step}.pt", history)

save_checkpoint("pinn_airfoil_v5_gauge_adam.pt", history)
...
save_checkpoint("pinn_airfoil_v5_gauge.pt", history)
export_onnx()
