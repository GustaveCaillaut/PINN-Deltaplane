"""
PINN airfoil open-boundary laboratory, V3.
...
run_pinn_openbc_suite_v3.py with --preset full or --preset overnight.
"""
...
def boundary_normals_from_sides(sides: torch.Tensor) -> torch.Tensor:
    """Outward normals for rectangle side codes: left=0, right=1, bottom=2, top=3."""
    normals = torch.zeros((sides.numel(), 2), dtype=torch.float32, device=sides.device)
    normals[sides == 0] = torch.tensor([-1.0, 0.0], device=sides.device)
    normals[sides == 1] = torch.tensor([ 1.0, 0.0], device=sides.device)
    normals[sides == 2] = torch.tensor([0.0, -1.0], device=sides.device)
    normals[sides == 3] = torch.tensor([0.0,  1.0], device=sides.device)
    return normals


def open_boundary_losses(
    model: nn.Module,
    xy_bc: torch.Tensor,
    bc_sides: torch.Tensor,
    u_inf: float,
    v_inf: float,
    eps_parallel: float = 0.15,
) -> Dict[str, torch.Tensor]:
    """Open-boundary loss used after discussion with the professor.

    Convention:
      - right boundary: pressure p=0 only, no velocity condition;
      - other boundaries are classified by s = Uhat · n_out;
      - if s < -eps: full inflow velocity u=U_inf, v=V_inf;
      - if |s| <= eps: side/slip boundary, normal velocity u·n = 0;
      - if s > eps: one scalar condition u·n = U_inf·n.

    This avoids imposing the freestream velocity on all four boundaries, which
    was one likely reason the pure PINN stayed too close to a uniform far-field.
    """
    out = model(xy_bc)
    vel = out[:, 0:2]
    p = out[:, 2:3]
    normals = boundary_normals_from_sides(bc_sides)

    U_vec = torch.tensor([u_inf, v_inf], dtype=torch.float32, device=xy_bc.device).view(1, 2)
    U_norm = math.sqrt(float(u_inf) ** 2 + float(v_inf) ** 2) + 1e-12
    U_hat = U_vec / U_norm
    s = torch.sum(normals * U_hat, dim=1, keepdim=True)

    right_mask = (bc_sides == 1).view(-1, 1)
    non_right = ~right_mask
    inflow_mask = (s < -eps_parallel) & non_right
    parallel_mask = (torch.abs(s) <= eps_parallel) & non_right
    out_normal_mask = (s > eps_parallel) & non_right

    zero = torch.tensor(0.0, dtype=torch.float32, device=xy_bc.device)

    if inflow_mask.any():
        vel_in = vel[inflow_mask.squeeze(1)]
        target = U_vec.expand_as(vel_in)
        inflow_vel = torch.mean((vel_in - target) ** 2)
    else:
        inflow_vel = zero

    vel_n = torch.sum(vel * normals, dim=1, keepdim=True)
    U_n = torch.sum(U_vec * normals, dim=1, keepdim=True)

    normal_terms = []
    if parallel_mask.any():
        normal_terms.append((vel_n[parallel_mask] - 0.0) ** 2)
    if out_normal_mask.any():
        normal_terms.append((vel_n[out_normal_mask] - U_n[out_normal_mask]) ** 2)
    if normal_terms:
        normal_flux = torch.mean(torch.cat([x.reshape(-1) for x in normal_terms]) ** 2) if False else torch.mean(torch.cat(normal_terms, dim=0))
    else:
        normal_flux = zero

    if right_mask.any():
        right_p = torch.mean(p[right_mask] ** 2)
    else:
        right_p = zero

    # Counts are useful diagnostics, stored as tensors to keep the return type simple.
    return {
        "inflow_vel": inflow_vel,
        "normal_flux": normal_flux,
        "right_p": right_p,
        "n_inflow": torch.tensor(float(inflow_mask.sum().item()), device=xy_bc.device),
        "n_parallel": torch.tensor(float(parallel_mask.sum().item()), device=xy_bc.device),
        "n_outnormal": torch.tensor(float(out_normal_mask.sum().item()), device=xy_bc.device),
        "n_right": torch.tensor(float(right_mask.sum().item()), device=xy_bc.device),
    }
...
class PINNCase:
    ...
    # legacy/four-sides mode uses w_bc_vel.
    # openbc mode uses w_inflow_vel, w_normal_flux and w_right_p.
    w_bc_vel: float = 10.0
    boundary_mode: str = "legacy"  # "legacy" or "openbc"
    eps_parallel: float = 0.15
    w_inflow_vel: float = 10.0
    w_normal_flux: float = 10.0
    w_right_p: float = 1.0
    w_wall: float = 50.0
    w_gauge: float = 1.0
    w_far_cp: float = 0.0
...
def train_case(case: PINNCase, outdir: str | Path, device="auto", surface: Optional[Dict] = None, save_model: bool = True) -> Dict:
    ...
    for step in range(1, case.n_steps + 1):
        ...
        outflow_mask = (bc_sides == 1) | (bc_sides == 3)  # legacy right/top pressure variant.
        outflow_cp = torch.mean(bc_out[outflow_mask, 2:3] ** 2) if outflow_mask.any() else torch.tensor(0.0, device=device)

        if case.boundary_mode == "openbc":
            obc = open_boundary_losses(model, xy_bc, bc_sides, u_inf, v_inf, eps_parallel=case.eps_parallel)
            # For logging compatibility, bc_vel means the full inflow velocity loss.
            bc_vel = obc["inflow_vel"]
            normal_flux = obc["normal_flux"]
            right_p = obc["right_p"]
        else:
            # Old formulation: impose the freestream velocity on all four outer boundaries.
            bc_vel = torch.mean((bc_out[:, 0:1] - u_inf) ** 2 + (bc_out[:, 1:2] - v_inf) ** 2)
            normal_flux = torch.tensor(0.0, device=device)
            right_p = torch.tensor(0.0, device=device)
        ...
        total = (
            ...
            case.w_inflow_vel * bc_vel * (1.0 if case.boundary_mode == "openbc" else 0.0) +
            case.w_normal_flux * normal_flux + case.w_right_p * right_p +
        ...
        if step == 1 or step % case.eval_every == 0 or step == case.n_steps:
            ...
            row = {
                ...
                "normal_flux": float(normal_flux.detach().cpu()),
                "right_p": float(right_p.detach().cpu()),
...
def make_case_plots(case_dir: Path, case: PINNCase, surface: Dict, hist: pd.DataFrame, final: Dict) -> None:
    if len(hist):
        ...
        for col in ["pde", "div", "near_pde", "bc_vel", "normal_flux", "right_p", "wall", "data", "kutta"]:
...
def make_cases(preset: str = "full", cp_data_csv: str = "") -> List[PINNCase]:
    """Return the new professor-discussion cases.

    These are not the old exhaustive failure suite.  They focus on the corrected
    outer boundary conditions:
      - inflow: full velocity where U_inf·n < -eps;
      - side/parallel: u·n=0;
      - outgoing non-right boundaries: u·n=U_inf·n;
      - right boundary: p=0 only.

    Two families are produced:
      30_* pure unsupervised PINN, no XFOIL data;
      31_* mixed data+PDE if cp_data_csv is provided.
    """
    if preset == "smoke":
        base = dict(n_steps=20, n_f=32, n_near=32, n_bc=32, n_wall=32, n_kutta=16, n_data=32,
                    width=24, depth=2, print_every=10, eval_every=10, lr=1e-3)
    elif preset == "overnight":
        base = dict(n_steps=16000, n_f=2048, n_near=2048, n_bc=1536, n_wall=768, n_kutta=256, n_data=1536,
                    width=128, depth=6, print_every=500, eval_every=500, lr=5e-4)
    else:
        base = dict(n_steps=6000, n_f=1024, n_near=1024, n_bc=1024, n_wall=512, n_kutta=256, n_data=768,
                    width=128, depth=6, print_every=500, eval_every=500, lr=5e-4)

    common = dict(
        alpha_deg=5.0,
        U=1.0,
        bounds=(-4.0, 6.0, -3.0, 3.0),
        boundary_mode="openbc",
        eps_parallel=0.15,
        # Disable legacy all-boundary velocity and legacy pressure terms.
        w_bc_vel=0.0,
        w_far_cp=0.0,
        w_outflow_cp=0.0,
        # New open-boundary terms.
        w_inflow_vel=10.0,
        w_normal_flux=10.0,
        w_right_p=1.0,
        # Interior/wall terms.
        ...
        w_near_div=20.0,
        w_wall=50.0,
        w_gauge=0.0,  # right pressure already fixes the gauge.
        **base,
    )

    cases: List[PINNCase] = []

    # 1) The clean non-supervised version, in continuity with our last PINN attempts.
    cases.append(PINNCase(name="30_openBC_unsupervised_Re200", nu=0.005, w_kutta=0.0, w_data=0.0, **common))
    cases.append(PINNCase(name="30_openBC_unsupervised_kutta_Re200", nu=0.005, w_kutta=2.0, w_data=0.0, **common))

    # 2) Optional higher-Re check, still unsupervised.  It helps answer whether the old failure was only due to Re=200.
    if preset != "smoke":
        cases.append(PINNCase(name="30_openBC_unsupervised_Re1000", nu=0.001, w_kutta=0.0, w_data=0.0, **common))
        cases.append(PINNCase(name="30_openBC_unsupervised_kutta_Re1000", nu=0.001, w_kutta=2.0, w_data=0.0, **common))

    # 3) Mixed data + PDE, only if XFOIL Cp data is provided.
    if cp_data_csv:
        # Weak PDE: data strongly anchors Cp, PDE/BC keep the field physically regular.
        mixed = dict(common)
        mixed.update(dict(
            w_pde=0.05,
            w_div=0.2,
            ...
            w_right_p=0.1,
            w_data=100.0,
            cp_data_csv=cp_data_csv,
        ))
        cases.append(PINNCase(name="31_openBC_xfoilData_weakPDE_Re200", nu=0.005, w_kutta=0.0, **mixed))
        cases.append(PINNCase(name="31_openBC_xfoilData_weakPDE_kutta_Re200", nu=0.005, w_kutta=2.0, **mixed))

        # Stronger PDE variant: useful to show the trade-off between fitting Cp data and satisfying residuals.
        if preset != "smoke":
            stronger = dict(common)
            stronger.update(dict(
                w_pde=0.2,
                w_div=1.0,
                w_near_pde=0.5,
                w_near_div=2.0,
            ...
            cases.append(PINNCase(name="31_openBC_xfoilData_strongerPDE_Re200", nu=0.005, w_kutta=0.0, **stronger))
