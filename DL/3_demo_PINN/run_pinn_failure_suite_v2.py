#!/usr/bin/env python3
"""Run a reproducible suite of PINN airfoil failure experiments.

Examples
--------
Smoke test:
    python -u run_pinn_failure_suite_v2.py --preset smoke --outdir runs_pinn_failure_v2_smoke --no-save-model

Full pre-run:
    python -u run_pinn_failure_suite_v2.py --preset full --outdir runs_pinn_failure_v2_full

With XFOIL Cp data-prior controls:
    python -u run_pinn_failure_suite_v2.py --preset full \
      --cp-data-csv xfoil_cp_dataset_naca2412_dense_Re200k/xfoil_cp_dataset.csv \
      --outdir runs_pinn_failure_v2_full_data
"""
...
from pinn_airfoil_failure_lab_v2 import (
...
def parse_args():
    ...
    p.add_argument("--outdir", default="runs_pinn_failure_v2_full")
