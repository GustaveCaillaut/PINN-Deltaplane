...
struct FAeroCoefficientsV2
{
    ...
    // Moment coefficient around the chosen reference point.
    // Positive sign follows the local 2D convention:
    // CM = sum(rx * dFy - ry * dFx).
    UPROPERTY(BlueprintReadOnly, Category = "Aero")
    float CM = 0.0f;
};
...
public:
    ...
    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Aero Moment")
    float MomentReferenceXPinn = 0.0f;

    UPROPERTY(EditAnywhere, BlueprintReadWrite, Category = "Aero Moment")
    float MomentReferenceY = 0.0f;
    ...
    UFUNCTION(BlueprintCallable, Category = "Aero NN")
    bool EvaluateAeroWithMoment(float AlphaRad, float& CL, float& CD, float& CM);
